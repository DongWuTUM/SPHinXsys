...
Real radius_mid_surface = 10.0;                           /** Radius of the inner wall of the cylinder. */
Real thickness_real = 0.04;                             /** Thickness of the cylinder. */
...
//----------------------------------------------------------------------
std::string full_path_to_geometry = "./input/half_sphere_with_hole.stl";
//----------------------------------------------------------------------
//	Basic geometry parameters and numerical setup.
//----------------------------------------------------------------------
Real scale = 1.0;

Vec3d domain_lower_bound(-10.4 * scale, 0.0 * scale, -10.4 * scale);
Vec3d domain_upper_bound(10.4 * scale, 9.8 * scale, 10.4 * scale);
//Vec3d domain_lower_bound(0.0 * scale, 0.0 * scale, 0.0 * scale);
//Vec3d domain_upper_bound(20.6 * scale, 9.8 * scale, 20.6 * scale);
Vec3d cycle_center(-10.3 * scale, 0.0 * scale, -10.3 * scale);
//Vec3d cycle_center(0.0 * scale, 0.0 * scale, 0.0 * scale);
...
StdVec<Vecd> observation_location = { Vecd(radius_mid_surface, 0.0, 0.0),
									  Vecd(0.0, 0.0, radius_mid_surface) };
...
std::vector<Vecd> point_forces{ Vec3d(F_full, 0.0, 0.0), Vec3d(-F_full, 0.0, 0.0),
								Vec3d(0.0, 0.0, F_full), Vec3d(0.0, 0.0, -F_full) };
std::vector<Vecd> reference_positions{ Vec3d(radius_mid_surface, 0.0, 0.0), Vec3d(-radius_mid_surface, 0.0, 0.0),
									   Vec3d(0.0, 0.0, -radius_mid_surface), Vec3d(0.0, 0.0, radius_mid_surface) };
...
int main(int ac, char* av[])
{
	...
	if (system.RunParticleRelaxation())
	{
		...
		relax_dynamics::ShellNormalDirectionPrediction shell_normal_prediction(cylinder_body_inner, thickness, cos(0.2 * Pi));
	...
	*/
	...
	while (GlobalStaticVariables::physical_time_ < end_time)
	{
		...
		while (integeral_time < output_period)
		{
			...
			ite++;
			dt = 0.2 * computing_time_step_size.exec();
			integeral_time += dt;
			GlobalStaticVariables::physical_time_ += dt;
		}
