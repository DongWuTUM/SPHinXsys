/**
* @file 	dw_3d_plate2.cpp
* @brief 	This is the benchmark test of the shell.
* @details  We consider large deformation of a square plate under stepping load.
...
//Vec3d n_0 = Vec3d(0.0, 0.0, 1.0);				  /** Pseudo-normal. */
//int particle_number = 80;						  /** Particle number in the direction of the length */
//Real particle_spacing_ref = PL / (Real)particle_number; /** Initial reference particle spacing. */
//int BWD = 8;									  /** Width of the boundary layer measured by number of particles. */
//Real BW = particle_spacing_ref * (Real)BWD;			  /** Boundary width, determined by specific layer of boundary particles. */
///** Domain bounds of the system. */
//BoundingBox system_domain_bounds(Vec3d(-BW, -BW, -0.5 * particle_spacing_ref),
//	Vec3d(PL + BW, PH + BW, 0.5 * particle_spacing_ref));
...
//
//Real q = 5.0 * Youngs_modulus * 1.0e-4; /** Total distributed load. */
...
int main()
{
	...
	SimpleDynamics<TimeStepPartInitialization1> initialize_external_force1(plate_body,
		makeShared<TimeDependentExternalForce>(Vec3d(0.0, 0.0, q / (PT * rho0_s))));
	SimpleDynamics<TimeStepPartInitialization2> initialize_external_force2(plate_body,
		makeShared<TimeDependentExternalForce>(Vec3d(0.0, 0.0, -q * PL * PH /
			(4.0 * (BWD * particle_number + BWD * BWD) * particle_spacing_ref * particle_spacing_ref * PT * rho0_s))
			));
	...
	*/
	...
	while (GlobalStaticVariables::physical_time_ < end_time)
	{
		...
		while (integeral_time < output_period)
		{
			...
			//initialize_external_force1.exec(dt);
			//initialize_external_force2.exec(dt);
