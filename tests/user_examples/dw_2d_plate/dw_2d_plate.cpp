...
//
//Real q = 6.25 * Youngs_modulus * 1.0e-4; /** Total distributed load. */

//------------------------------------------------------------------------------
// global parameters for the case
//------------------------------------------------------------------------------
Real PL = 0.2;									  /** Length of the square plate. */
Vec2d n_0 = Vec2d(0.0, 1.0);					  /** Pseudo-normal. */
Real thickness = 0.001;							  /** Thickness of the square plate. */
//int particle_number = 1;						  /** Particle number in the direction of the thickness */
int particle_number_length = 40;	/** Particle number in the direction of the length */
Real resolution_ref = PL / (Real)particle_number_length; /** Initial reference particle spacing. */
//int BWD = 0.06 / resolution_ref;									  /** number of boundary particles layers . */
int BWD = 1;									  /** number of boundary particles layers . */
Real BW = resolution_ref * (Real)BWD;			  /** Boundary width, determined by specific layer of boundary particles. */
BoundingBox system_domain_bounds(Vec2d(-BW, -0.5 * resolution_ref), Vec2d(PL + BW, 0.5 * resolution_ref));
StdVec<Vecd> observation_location = { Vecd(PL + 1.5 * resolution_ref, 0.0) };
//----------------------------------------------------------------------
//	Material properties of the fluid.
//----------------------------------------------------------------------
Real rho0_s = 1000.0;				   /** Normalized density. */
Real Youngs_modulus = 2.0e6; /** Normalized Youngs Modulus. */
...
public:
	...
	{
		// the plate and boundary
		for (int i = 0; i < (particle_number_length + BWD); i++)
		{
			Real x = resolution_ref * i - BW + resolution_ref * 0.5;
			initializePositionAndVolumetricMeasure(Vecd(x, 0.0), resolution_ref);
			initializeSurfaceProperties(n_0, thickness);
...
private:
	void tagManually(size_t index_i)
	{
		if (base_particles_.pos_[index_i][0] < 0.0)
		{
			body_part_particles_.push_back(index_i);
		}
	};
...
int main()
{
	...
	plate_observer.defineAdaptationRatios(1.15, 2.0);
	...
	//----------------------------------------------------------------------
	SimpleDynamics<BeamInitialCondition> plate_initial_velocity(plate_body);
	...
	//	makeShared<TimeDependentExternalForce>(Vec2d(0.0, q / (thickness * rho0_s))));
