...
protected:
	...
	int particle_number = 240;								 /** Particle number in the bottom edge. */
...
public:
	...
	virtual void initializeGeometricVariables() override
	{
		...
		for (int i = 0; i < particle_number; i++)
		{
			...
			{
				Real x = radius_mid_surface * sin(i * 2 * Pi / (Real)particle_number);
				Real y = particle_spacing_ref * j - BW;
				Real z = radius_mid_surface * cos(i * 2 * Pi / (Real)particle_number);
...
private:
	void tagManually(size_t index_i)
	{
		...
		if (solid_particles.pos0_[index_i][0] < 0.5 * particle_spacing_ref
			&& solid_particles.pos0_[index_i][0] > -0.5 * particle_spacing_ref
			&& solid_particles.pos0_[index_i][2] > 0.0)
		{
...
protected:
	...
	SimpleDynamics<solid_dynamics::FixedInAxisDirection> constrain_holder;
...
public:
	explicit Environment(Real loading_factor_new) :
		...
		constrain_holder(boundary_geometry, Vecd(0.0, 1.0, 1.0)),
		...
		point_forces({ Vec3d(0.0, 0.0, loading_factor * 1000.0), Vec3d(0.0, 0.0, -loading_factor * 1000.0) }),
...
PYBIND11_MODULE(dw_3d_pinched_cylinder_python, m)
{
	py::class_<Environment>(m, "pinched_cylinder_from_sph_cpp")
