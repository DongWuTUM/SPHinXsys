...
//----------------------------------------------------------------------
Real scale = 1.0e-3;
Real F_full = 25.0e3; /** point force. */
Vec3d point_force_pos_upper = Vec3d(0.0025, 0.0, 0.005);
Vec3d point_force_pos_under = Vec3d(0.0025, 0.0, -0.005);
Vec3d force_upper = Vec3d(0.0, 0.0, -F_full);
Vec3d force_under = Vec3d(0.0, 0.0, F_full);
Real total_physical_time = 0.006;
Vec3d domain_lower_bound(-0.01, -0.005, -0.005);
Vec3d domain_upper_bound(0.01, 0.005, 0.005);
Real dp_0 = 0.1 * 1.0e-3;
Real thickness = 0.1 * 1.0e-3;
Real resolution_ref = 0.25 * d; //When resolution_ref = 0.25 * d, updated_indicator_[particles_->total_real_particles_ / 5] = 1 in relax_dynamics.cpp;
Real center = 261.3234 * scale;
Real constrained_circle = powerN(center - 3.0 * resolution_ref, 2); //When resolution_ref = 0.25 * d
//Real constrained_circle = powerN(center - 1.5 * resolution_ref, 2); //When resolution_ref = 0.5 * d
...
public:
	...
	{
		add<TriangleMeshShapeSTL>("./input/ps_stent.stl", Vecd(0.0) * scale, scale);
	}
};
...
int main(int ac, char *av[])
{
	...
	plate_body.addBodyStateForRecording<Matd>("PseudoNormalCorrectionMatrix");
