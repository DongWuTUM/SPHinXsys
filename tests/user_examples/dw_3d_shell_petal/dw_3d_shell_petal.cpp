...
{// Units: g, mm, ms, N, MPa
	...
	constexpr double time_unit = 2e3; // s to ms
...
void shell_bending()
{
	...
	const double acc_max = 100.0 * unit_system::acceleration_unit;
	const double height_fix = 8 * scale;
	const double height_pressure_min = 35 * scale;
	...
	shell_body.addBodyStateForRecording<Vec3d>("PseudoNormal");
	shell_body.addBodyStateForRecording<Vec3d>("PriorAcceleration");
	...
	int ite = 0;
	const double output_period = end_time / 300.0;
	double dt = 0.0;
	...
	try
	{
        ...
        TickCount::interval_t tt = TickCount::now() - t1;
