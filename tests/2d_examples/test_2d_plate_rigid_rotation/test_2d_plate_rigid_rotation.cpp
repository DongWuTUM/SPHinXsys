...
Real sqrt_2_divide_2 = sqrt(2.0) / 2.0;
//Vec2d n_0 = Vec2d(sqrt_2_divide_2, sqrt_2_divide_2);					  /** Pseudo-normal. */
...
public:
	...
	virtual void initializeGeometricVariables() override
	{
		...
		for (int i = 0; i < (particle_number + 2 * BWD); i++)
		{
			...
			//initializePositionAndVolumetricMeasure(Vecd(-sqrt_2_divide_2 * position, sqrt_2_divide_2 * position), resolution_ref);
...
protected:
	...
	void update(size_t index_i, Real dt = 0.0)
	{
		...
		//vel_[index_i] = Vecd(0.0, -10.0 * pos0_[index_i][1]);
...
int main()
{
	...
	int ite = 0;
	Real end_time = 2.0;
	Real output_period = end_time / 2000.0;
	Real dt = 0.0;
